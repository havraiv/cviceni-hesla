...
const createAccount = (user, generatePassword) => {
  return `<p>Vaše uživatelské jméno je: ${user}</p>
  <p>Vaše heslo je: ${generatePassword}</p>`;
};

document.body.innerHTML = createAccount('Martina', strongPassword(50));
...
document.body.innerHTML += `<p>${createAccount('Míša', weakPassword(9))}</p>`;
document.body.innerHTML += `<p>${createAccount(
  'Řízek',
  mediumPassword(9),
)}</p>`;
document.body.innerHTML += `<p>${createAccount(
